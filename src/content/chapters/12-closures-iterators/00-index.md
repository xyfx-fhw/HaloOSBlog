...
| 文章 | 主要内容 |
| --- | --- |
| [闭包](./01-closures) | 闭包语法、三种捕获方式（借用/可变借用/移动），以及何时用 `move` |
| [Fn Trait](./02-fn-traits) | `Fn`/`FnMut`/`FnOnce` 三个 trait 的区别，闭包作为参数与返回值 |
| [迭代器基础](./03-iterators) | 惰性求值、`iter`/`into_iter`/`iter_mut`、自定义迭代器与零开销原理 |
| [适配器](./04-adaptors) | 消费适配器与迭代器适配器的本质区别，常用方法速查 |
| [综合练习](./05-practice) | 手写 `my_filter` 和 `my_sum`，理解标准库适配器的实现原理 |
