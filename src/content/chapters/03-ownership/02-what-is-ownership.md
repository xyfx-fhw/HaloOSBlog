...
Rust 的答案是：**不允许这种情况发生**。所有权规则从根本上杜绝了这个问题，从上一节的”移动、拷贝、克隆”可以看出来：

- **移动**：所有权转移，原所有者失效。永远只有一个有效的所有者指向堆数据，因此只会释放一次。
- **拷贝**：栈类型自动复制值。栈上的数据没有释放的概念（栈帧离开自动清理），不存在释放冲突。
- **克隆**：显式深拷贝。通过 `.clone()` 的形式让程序员明确知道”这是成本操作”，创建完全独立的副本，各自管理各自的内存。
