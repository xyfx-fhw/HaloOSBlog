...
接下来，我们先学习一下栈和堆，移动、拷贝与克隆这两个基础知识，方便大家更好的理解后续所有权的原理
...
**判断一个类型是 Move 还是 Copy 的快捷方法**：如果它需要在堆上分配内存（`String`、`Vec`、`Box` 等），通常是 Move；如果它只存在栈上（整数、浮点、布尔、char、小元组），通常是 Copy。
