...
Rust 有两种文档注释，分别注释**它们所在位置的不同对象**：

### `///`：外部文档注释

`///` 写在一个 item（函数、结构体、枚举、字段等）**之前**，是那个 item 的文档：

```rust runnable
/// 计算两个整数的和。
///
/// 这个函数对任意 i32 范围内的整数都有效。
...
`//!` 写在模块或文件的**最顶部**（内部），是**包含它的 item**（通常是模块或 crate）的文档：
...
Rust 社区约定了几个标准章节名，`cargo doc` 会把它们格式化得更显眼：
...
`cargo doc` 会在 `target/doc/` 目录下生成完整的 HTML 文档。发布到 crates.io 的 crate 文档会自动部署到 [docs.rs](https://docs.rs)。
...
Doctest 里默认没有 `main()` 函数，也没有 `?` 运算符的错误传播上下文。如果示例需要用 `?`，有两种方式：
...
/// # Examples
///
/// 可以使用 ? 运算符传播错误（注意返回类型的写法）：
///
/// ```
/// # use std::num::ParseIntError;
/// fn try_parse() -> Result<(), ParseIntError> {
///     let n: i32 = "42".parse()?;
///     assert_eq!(n, 42);
///     Ok(())
/// }
...
+ no_run（编译但不运行）
+ ignore（跳过这个测试）
+ should_panic（期望代码 panic）
+ compile_fail（期望编译失败）
- runnable（这是本教程特有的，不是 Rust 官方 doctest 语法）
...
下面有一个格式化温度的函数，但文档不完整。请补全文档注释，使其包含：描述、`# Examples` 章节，以及至少一个可通过 doctest 验证的断言。
...
///
/// ```
/// // TODO: 写一个示例，验证 format_temp(36.6, "C") 返回 "36.6°C"
/// ```
pub fn format_temp(value: f64, unit: &str) -> String {
    format!("{:.1}°{}", value, unit)
}

fn main() {
    println!("{}", format_temp(36.6, "C"));
    println!("{}", format_temp(98.2, "F"));
}
```
