...
> **重要提示**：**不能在遍历 HashMap 时修改其大小**（添加或删除键值对）。这会导致迭代器失效，导致编译错误。如果需要在遍历中过滤或修改 HashMap，应该先遍历收集结果，然后在循环外修改。这个限制和向量一样——它们都使用迭代器，都要保护迭代器的有效性。
...
```expected
最常见的单词：'programming' 出现 3 次
```

### 练习 4：分组统计

给定一个人名列表和他们的部门，创建一个 HashMap 来存储每个部门的员工列表。
...
```expected
Engineering 部门有 3 人：["Alice", "Charlie", "Frank"]
Sales 部门有 2 人：["Bob", "Eve"]
HR 部门有 1 人：["Diana"]
```

### 练习 5：HashMap 综合应用——用户信息管理

创建一个用户数据库，存储用户名和他们的信息（年龄和邮箱）。实现查询、添加和删除用户的操作。

```rust editable
use std::collections::HashMap;

// TODO: 定义一个 User 结构体，包含 age（u32）和 email（String）
