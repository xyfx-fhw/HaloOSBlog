...
**单元类型** `()` 表示"无值"或"空"，是 Rust 中一个合法的类型。不要把它和其他语言的 `null`、`void` 或 `None` 混淆——`()` 是一个真实的值，可以赋给变量、打印、传递给函数。
...
切片 `&[T]` 是对数组（或数组的一段）的**引用视图**，大小在编译时不固定（切片使用的都是原数据的引用，内存里只有一份原数据）：
