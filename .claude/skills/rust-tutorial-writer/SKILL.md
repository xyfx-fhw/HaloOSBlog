...
- **放在第一个 `#` 之前**：仅限所有 Tab 都需要的极简导语（1-2 句话），如"本文讲解 X"或"你将学到 Y"。**不要放详细内容**。
- **放在第一个 `#` 之后**：属于该 Tab 的内容，包括该 Tab 的正式介绍段落、代码示例、说明文字等。

> **常见错误**：把第一个 Tab 的介绍段落写在第一个 `#` 之前 → 导致每个 Tab 都重复显示这段文字，体验很差。**内容一旦只属于某个 Tab，就必须写在该 Tab 的 `#` 后面。**
...
**H2 标题**：直接写题目主题，不加"单选："/"多选："前缀，类型由 quiz 块内部的 `single`/`multi` 标识。
