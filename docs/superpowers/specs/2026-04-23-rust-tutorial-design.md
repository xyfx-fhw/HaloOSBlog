...
**日期**：2026-04-23
**状态**：实现中（子计划 1–5 已完成，子计划 6–7 待实现）
...
| 4 | 进度系统（localStorage 进度追踪，进度管理集成在首页） | ✅ 完成 |
...
构建一个名为 **HaloOS 系列讲解** 的网页教学平台，面向完全零基础的 Rust 学习者（即使有 C 语言背景，也从零开始讲解所有 Rust 概念）。核心目标是提供像官方 Rust Book 一样系统的讲解，同时加入在线代码执行、可编辑练习、章节测验等互动功能。
...
| 主要文字 | `#E8E8E8` |
| 正文段落文字 | `#C0C5D0`（比 `--color-text` 略暗偏冷，避免纯白刺眼） |
| 次要文字 | `#888890` |
| 中文字体 | `Noto Sans SC`（优先）/ `PingFang SC` / `微软雅黑` |
| 英文 / 代码字体 | `Inter`（正文）/ `JetBrains Mono`（代码） |
| 内容最大宽度 | `720px`（文章正文），居中，不撑满全宽 |
| **`**bold**` 加粗** | 主题橙 `var(--color-accent)` (#CE412B)，字重 700 |
| **`__bold__` 加粗** | 渐变色：玫瑰 `#f43f5e` → 橙 `#f97316` → 琥珀 `#eab308`，`background-clip: text`，字重 700 |
...
**章节文档页底部按钮：** ~~"进入练习"和"进入测验"按钮已废弃~~——练习以同级 Markdown 文章的形式出现（如 `1.3 练习：入门基础`），通过普通的上一节/下一节导航串联，不再需要独立跳转按钮。
...
```
HaloOS 系列讲解    首页  教程    ⏱ 38%
```

- Logo：文字 "HaloOS 系列讲解"（RUST 全大写）
...
| 模块 | 内容 |
|------|------|
| Hero 区 | 课程名称、简介、"开始学习"按钮（→ 上次阅读文章，无记录则第一章），按钮下方显示作者署名"作者：雪云飞星"，背景有动态光晕 |
...
- 进度条（`flex:1`）横跨"课程目录"标签与右侧指示器之间，撑满整行
- 右侧指示器使用 CSS grid 叠放两个元素：
  - 默认显示：`总进度 X%`（等宽小字）
  - 有进度时悬停：百分比淡出，红色"↺ 重置全部进度"按钮叠入（与单条重置按钮同款样式）
  - 无进度时：不触发悬停互换
- 重置全部进度需弹出确认框，手动输入"重置全部进度"后方可执行
...
进度指示器（右侧）使用 CSS grid 叠放：

- 默认显示：彩色进度灯（空心圆 = 未开始，黄边 = 进行中，绿实 = 已完成）
- 有进度时悬停到进度灯：进度灯淡出，红色"↺ 重置进度"按钮叠入
- 无进度时：不触发悬停互换，进度灯纯展示

按钮样式（单条与全部重置统一）：红色文字 `var(--color-error)`、半透明红色边框、hover 时加深红色背景，`font-size: 0.75rem`。
...
### 8.4 ~~学习进度页（`/progress`）~~ ← 已废弃

> ⚠️ **已删除**：独立的 `/progress` 页面已废弃，进度查看与重置功能全部集成在首页课程目录中（见 8.3）。
...
每篇文章权重相等（1 份）。有 H2 的文章按已读小节占比折算（例如 3 节读了 2 节 = 0.67 份）；无 H2 的文章整篇完成得满分（1 份）；未访问的文章贡献 0。

```text
总进度 = Σ(每篇文章的小节完成比) / 总文章数 × 100
```

| 文章类型 | 计分方式 |
| -------- | -------- |
| 有 H2，部分已读 | 已读小节数 / 总小节数 |
| 有 H2，全部已读 | 1（满分） |
| 无 H2，已完成 | 1（满分） |
| 未访问 / 未开始 | 0 |
